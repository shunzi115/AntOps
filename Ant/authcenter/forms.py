...
from authcenter.models import UserInfo, RoleList, PermissionList

class LoginUserForm(forms.Form):
    email = forms.EmailField(label=u'邮 箱', error_messages={'required': u'账号不能为空'},
...
class AddUserForm(forms.ModelForm):
    class Meta:
        model = UserInfo
        fields = ('name', 'email','password','department', 'mobile', 'role', 'is_active')
        widgets = {
    ...
    def __init__(self,*args,**kwargs):
        ...
        self.fields['name'].error_messages = {'required': u'请输入名称', 'invalid': u'请输入有效名称'}
        ...
        self.fields['department'].label = u'部 门'
        self.fields['department'].error_messages = {'required': u'请输入部门'}
        self.fields['mobile'].label = u'手 机'
        self.fields['mobile'].error_messages = {'required': u'请输入手机'}
    ...
    def clean_password(self):
        password = self.cleaned_data.get('password')
        if len(password) < 8:
            raise forms.ValidationError(u'密码必须大于8位')
        return password

class EditUserForm(forms.ModelForm):
    class Meta:
        model = UserInfo
        fields = ('name', 'email', 'department', 'mobile', 'role', 'is_active')
        widgets = {
            ...
            'department': forms.TextInput(attrs={'class':'form-control', 'style': 'width:500px;'}),
            'mobile': forms.TextInput(attrs={'class':'form-control', 'style': 'width:500px;'}),
